...
import {
  ...
  hasContactInfo,
  isValidProfile,
} from "../scrapping/profile-cleaner";
...
async function stage3_ScrapeBatches(jobId: string) {
  const stageStart = Date.now();
  console.log(`\n${"─".repeat(80)}`);
  console.log(`⚙️  STAGE 3: SCRAPE PROFILES (BATCHED)`);
  console.log(`${"─".repeat(80)}`);
  ...
  try {
    ...
    if (job.lastScrapedBatch >= totalBatches) {
      ...
      await prisma.sourcingJob.update({
        where: { id: jobId },
        data: { status: "PARSING_PROFILES", currentStage: "PARSING_PROFILES" },
      });
      return;
    }
    ...
    for (let i = job.lastScrapedBatch; i < totalBatches; i++) {
      ...
      console.log(
        `✓ Scraped ${succeeded}/${rawProfiles.length} profiles successfully`
      );

      // ✅ CHECKPOINT: Save scraped data for this batch
      scrapedData[`batch_${i}`] = rawProfiles;
...
// ============================================================================
// STAGE 4: Parse Profiles with AI in Batches
// ============================================================================

async function stage4_ParseBatches(jobId: string) {
  const stageStart = Date.now();
  console.log(`\n${"─".repeat(80)}`);
  console.log(`🤖 STAGE 4: PARSE PROFILES WITH AI (BATCHED)`);
  console.log(`${"─".repeat(80)}`);
  ...
  try {
    ...
    // Check if already completed
    if (job.lastParsedBatch >= totalBatches) {
      console.log(`✅ Stage 4 already completed (all batches parsed)`);
      await prisma.sourcingJob.update({
        where: { id: jobId },
    ...
    const parsedData: Record<string, any[]> =
      (job.parsedProfilesData as any) || {};

    // Process batches from where we left off
    for (let i = job.lastParsedBatch; i < totalBatches; i++) {
      const batchKey = `batch_${i}`;
      const rawProfiles = scrapedData[batchKey];
      ...
      console.log(
        `\n🔄 Parsing Batch ${i + 1}/${totalBatches} (${
          rawProfiles.length
        } profiles)...`
      );
      ...
      await prisma.sourcingJob.update({
        where: { id: jobId },
        data: {
          currentStage: `PARSING_BATCH_${i + 1}`,
          lastActivityAt: new Date(),
        },
      });

      // Clean and parse profiles
      const cleanedProfiles = rawProfiles
        .map(cleanProfileData)
        .filter(isValidProfile);

      console.log(
        `✓ Cleaned: ${cleanedProfiles.length}/${rawProfiles.length} valid`
      );
      ...
      const parsedProfiles = parseResults
        .filter((r) => r.status === "fulfilled")
        .map((r: any) => r.value);

      console.log(
        `✓ Parsed: ${parsedProfiles.length}/${cleanedProfiles.length} successful`
      );

      // ✅ CHECKPOINT: Save parsed data for this batch
      parsedData[batchKey] = parsedProfiles;

      await prisma.sourcingJob.update({
        where: { id: jobId },
        data: {
          parsedProfilesData: parsedData as any,
          lastParsedBatch: i + 1,
          profilesParsed: { increment: parsedProfiles.length },
          lastActivityAt: new Date(),
        },
      });
    ...
    const duration = ((Date.now() - stageStart) / 1000).toFixed(2);
    console.log(`\n✅ Stage 4 COMPLETE - All batches parsed (${duration}s)\n`);
  } catch (error: any) {
...
async function stage5_SaveBatches(jobId: string) {
  ...
  try {
    ...
    if (Object.keys(parsedData).length === 0) {
      throw new Error("No parsed data found. Run Stage 4 first.");
    }
    ...
    for (let i = job.lastSavedBatch; i < totalBatches; i++) {
      const batchKey = `batch_${i}`;
      const profiles = parsedData[batchKey];

      if (!profiles || profiles.length === 0) {
        console.log(`⚠️  Batch ${i + 1} has no parsed data, skipping...`);
        await prisma.sourcingJob.update({
          where: { id: jobId },
