...
export async function processSourcingJobWithCheckpoints(jobId: string) {
  ...
  try {
    ...
    if (job.lastCompletedBatch !== null && job.lastCompletedBatch !== undefined) {
      console.log(`   - 🔄 Resuming from batch: ${job.lastCompletedBatch + 1}`);
    }
...
async function formatJobDescription(job: any) {
  ...
  try {
    ...
    console.log(`      - Keywords: ${searchFilters.keywords?.length || 0}`);
    console.log(`      - Titles: ${searchFilters.titles?.length || 0}`);
    console.log(`      - Experience: ${searchFilters.experienceYears || 'N/A'}`);
    console.log(`      - Location: ${searchFilters.location || 'N/A'}`);
    console.log(`      - Companies: ${searchFilters.companies?.length || 0}`);
