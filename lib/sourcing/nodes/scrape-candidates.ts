...
export async function scrapeCandidates(state: SourcingState) {
  ...
  const remainingCandidates = candidates.filter(
    c => !alreadyScrapedUrls.has(c.profileUrl)
  );
  ...
  console.log(`🔄 Scraping ${remainingCandidates.length} remaining profiles...`);
  ...
  for (let i = 0; i < totalBatches; i++) {
    ...
    const urls = batch.map(c => c.profileUrl);

    console.log(`📦 Scraping batch ${i + 1}/${totalBatches} (${urls.length} profiles)...`);
